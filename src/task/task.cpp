...
Task::Task(char prefix, std::string &description)
...
std::ostream &operator<<(std::ostream &stream, Task &task) {
    /* TODO-SAM Investigate why this has to be copied to be passed to stream */
    Metadata metadata = task.getMetadata();
    stream << metadata << task.getPrefix() << " " << task.getDescription();
    return stream;
}
